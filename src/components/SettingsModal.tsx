...
interface LoadedState extends FormState {
  hasExistingSettings: boolean;
}

function loadInitialValues(): LoadedState {
...
function SettingsModalContent({ onClose }: { onClose: () => void }) {
  ...
  const selectedProvider = useMemo(() => {
    return getProviderDefinition(formState.provider);
  }, [formState.provider]);
  ...
  return (
    <>
      <button
        type="button"
        onClick={onClose}
        className="absolute top-16 right-16 flex h-32 w-32 items-center justify-center rounded-full text-neutral-400 transition-colors hover:bg-neutral-100 hover:text-neutral-600"
        aria-label="关闭"
      >
        <Icon name="X" size={20} strokeWidth={1.5} />
      </button>

      <h1 className="text-foreground mb-32 font-serif text-[24px] font-normal">设置</h1>
...
export function SettingsModal() {
  ...
  return (
    ...
    >
      <div className="relative max-h-[90vh] w-full max-w-[480px] overflow-y-auto rounded-2xl bg-white p-32 shadow-xl">
        <SettingsModalContent onClose={closeSettings} />
      </div>
    </div>
  );
}
