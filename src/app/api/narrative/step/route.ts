...
import { buildStepPrompt } from '@/lib/prompts/narrative-v1';
...
export async function POST(request: Request): Promise<Response> {
  ...
  const { provider, apiKey, model, baseUrl, symbolId, element, history, choiceId, choiceText } =
    body;
  ...
  if (!symbolId || !element || !history || !choiceId || !choiceText) {
  ...
  const messages = buildStepPrompt(context, history, choiceId, choiceText);
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        send({ type: 'done', raw: accumulated });
      } catch (err) {
        if (err instanceof LLMAuthError) {
          ...
          send({ type: 'error', message: '叙事生成失败' });
        }
      } finally {
        controller.close();
      }
